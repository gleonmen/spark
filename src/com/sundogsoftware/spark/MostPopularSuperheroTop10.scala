...
object MostPopularSuperheroTop10 {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("C:/proyectos/02. Nilseen/SparkScala/marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-apperarance data
    val lines = sc.textFile("C:/proyectos/02. Nilseen/SparkScala/marvel-graph.txt")
    ...
    val top10 = flipped.sortByKey(false).take(10).foreach(x => println(namesRdd.lookup(x._2)(0)))
    print("less populars\n")
    val nonPopular10 = flipped.sortByKey(true).take(10).foreach(x => println(namesRdd.lookup(x._2)(0)))
